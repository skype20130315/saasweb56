	...
	public ReturnObject ExecOther(String actionType, Object[] o) {
		...
		if( "getDetail".equals( actionType ) ){
			try{
				...
				StringBuffer countSql = new StringBuffer( "select count(distinct(GS.GSID)) amount from INF_GOODS G left join INF_GOODSUPLY GS on G.GDSGCODE=GS.GSSGCODE AND G.GDID = GS.GSID " );
				StringBuffer sql = new StringBuffer( "select distinct (GS.GSID), G.*, GS.GSSUPID, SUP.SUPNAME from INF_GOODS G left join INF_GOODSUPLY GS on G.GDSGCODE = GS.GSSGCODE AND G.GDID = GS.GSID LEFT JOIN INF_SUPINFO SUP ON GS.GSSUPID = SUP.SUPID AND G.GDSGCODE = SUP.SUPSGCODE " );
				...
				if(!StringUtil.isBlank(infGoods.getSupcode())){
					whereStr.append( " and GS.GSSUPID = '" ).append( infGoods.getSupcode() ).append("'");
				}
				...
				log.error("GoodsManagerImpl.getCategory() error : "
						+ ex.getMessage());
