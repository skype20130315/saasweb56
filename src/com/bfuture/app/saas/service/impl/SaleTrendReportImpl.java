...
//								else 
		...
		}else if( "getChart_YPDM".equals( actionType ) ){
			try{
				YwGoodssale goodsale = (YwGoodssale)o[0];
				StringBuffer chartSql =new StringBuffer( "SELECT SYSDATE RQ,TO_CHAR(T.GSRQ,'yyyy-MM-DD') GSRQ,SUM(GSXSSL) GSXSSL FROM YW_GOODSSALE_M T WHERE 1 = 1  " );
				if( !StringUtil.isBlank( goodsale.getGssupid() ) ){
						chartSql.append( " and T.GSSUPID = '" ).append( goodsale.getGssupid() ).append("'");
				}
...
//								else 
		...
		}else if ("getSaleTrendReport_YPDM".equals( actionType )) {
			result = getSaleTrendReport_YPDM(o);
		}
	...
	public ReturnObject getSaleTrendReport(Object[] o) {
		...
		try{
			...
			// chartSql.append( " GROUP BY T.GSRQ,T1.SHPCODE,T1.SHPNAME " );		
			// to_char(T.GSRQ,'YYYY-MM-DD'), case when T1.SHPCODE is null then 'unknow' else T1.SHPCODE end, case when T1.SHPNAME is null then '其他' else T1.SHPNAME end, T2.supid, T2.supname
	...
	public ReturnObject getSaleTrendReport_YPDM(Object[] o) {
		ReturnObject result = new ReturnObject();
		log.info("SaleTrendReportImpl.getSaleTrendReport_YPDM()");
		try{
			YwGoodssale goodsale = (YwGoodssale)o[0];
			StringBuffer chartSql = new StringBuffer( "SELECT to_char(T.GSRQ,'YYYY-MM-DD') GSRQ,case when T1.SHPCODE is null then 'unknow' else T1.SHPCODE end SHPCODE,case when T1.SHPNAME is null then '其他' else T1.SHPNAME end SHPNAME,SUM(GSXSSL) GSXSSL,SUM(GSHSJJJE) GSHSJJJE,SUM(GSXSSR) GSXSSR, T2.supid INFSUPID,T2.supname INFSUPNAME FROM YW_GOODSSALE_M T  " );				
				chartSql.append( " LEFT JOIN INF_SHOP T1 ON T.GSMFID=T1.SHPCODE AND T.GSSGCODE = T1.SGCODE left join inf_supinfo T2 on T2.supsgcode = T.gssgcode and T2.supid = T.gssupid WHERE 1 = 1 " );
