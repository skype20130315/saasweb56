...
public class YwZrstockDetailsManagerImpl extends BaseManagerImpl implements YwZrstockDetailsManager {
	...
	public ReturnObject getResult(Object o) {
		...
		try{
			...
			StringBuffer countSql = new StringBuffer( "select count(*)  FROM "+TableName+" Z left join INF_GOODS G on Z.ZSSGCODE=G.GDSGCODE  and Z.ZSGDID=G.GDID left join INF_SHOP P on G.GDSGCODE=P.SGCODE and Z.ZSMFID=P.SHPCODE left JOIN INF_SUPINFO S ON S.SUPSGCODE=Z.ZSSGCODE AND S.SUPID=Z.ZSSUPID " );			
			StringBuffer sql = new StringBuffer( "select (sysdate-1) as ZSGXTIME,ZSJYFS,P.SHPCODE,P.SHPNAME,G.GDID,G.GDBARCODE,G.GDNAME,G.GDSPEC,G.GDUNIT,Z.TEMP1,S.SUPID,S.SUPNAME,Z.ZSKCSL,Z.ZSKCJE FROM "+TableName+" Z left join INF_GOODS G on Z.ZSSGCODE=G.GDSGCODE  and Z.ZSGDID=G.GDID left join INF_SHOP P on G.GDSGCODE=P.SGCODE and Z.ZSMFID=P.SHPCODE left JOIN INF_SUPINFO S ON S.SUPSGCODE=Z.ZSSGCODE AND S.SUPID=Z.ZSSUPID " );
			...
			if( !StringUtil.isBlank( stock.getZsgdid() ) ){// [商品编码]
				whereStr.append( " and G.GDID = '" ).append( stock.getZsgdid() ).append("'");
			}
			if( !StringUtil.isBlank( stock.getGdbarcode() ) ){//商品条码
				whereStr.append( " and G.GDBARCODE like '%" ).append( stock.getGdbarcode() ).append("%'");
			}
