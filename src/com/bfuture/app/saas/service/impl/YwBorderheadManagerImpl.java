    ...
    implements YwBorderheadManager
{
    ...
    public ReturnObject SearchYwBorderhead(Object o[])
    {
        ...
        try
        {
        	...
        	sql.append(" yb.bohjyfs,yb.bohgz,yb.bohmemo,yb.temp1,yb.temp2,yb.temp3,yb.temp4,yb.temp5,sta.BOHSTATUS BOHSTATUS,sp.SHPNAME,sp.TEMP1 LINKTELE,sp.TEMP2 ADDRESS,su.SUPNAME SUNAME,yd.SL,yd.JE ,su.supcont ,su.supfax ,su.supadd ,su.supcontphone from YW_BORDERHEAD yb left join INF_SHOP sp on yb.bohsgcode = sp.SGCODE ");
        	sql.append(" and yb.bohmfid = sp.SHPCODE left join INF_SUPINFO su on yb.bohsgcode = su.supsgcode and yb.BOHSUPID = su.supid left join YW_BORDERSTATUS sta on sta.bohsgcode=yb.bohsgcode and sta.bohbillno=yb.bohbillno and sta.bohshmfid = yb.bohmfid left join (select bodsgcode,bodbillno,bodshmfid,sum(bodsl) SL,sum(bodhsjjje) JE ");
            sql.append("  from yw_borderdet where 1=1 and bodsgcode = '").append(yb.getBohsgcode()).append("' group by bodsgcode,bodshmfid,bodbillno) yd on yb.bohsgcode=yd.bodsgcode and yb.bohbillno=yd.bodbillno and yb.bohmfid=yd.bodshmfid ");
            StringBuffer countSql = new StringBuffer("select count(*) from YW_BORDERHEAD yb left join YW_BORDERSTATUS sta on sta.bohsgcode=yb.bohsgcode and sta.bohbillno=yb.bohbillno and sta.bohshmfid = yb.bohmfid left join (select bodsgcode,bodbillno,bodshmfid,sum(bodsl) sl,sum(bodhsjjje) je from yw_borderdet ");
            StringBuffer sumSql =  new StringBuffer("select cast('\u5408\u8BA1' as varchar2(32)) BOHBILLNO,SUM(SL) SL,SUM(JE) JE,'4' BOHSTATUS from YW_BORDERHEAD yb left join YW_BORDERSTATUS sta on sta.bohsgcode=yb.bohsgcode and sta.bohbillno=yb.bohbillno and sta.bohshmfid = yb.bohmfid left join (select bodsgcode,bodbillno,bodshmfid,sum(bodsl) sl,sum(bodhsjjje) je from yw_borderdet ");
            countSql.append(" where 1=1 and bodsgcode='").append(yb.getBohsgcode()).append("' group by bodsgcode,bodshmfid,bodbillno) yd on yb.bohsgcode=yd.bodsgcode and yb.bohbillno=yd.bodbillno and yb.bohmfid=yd.bodshmfid ");
            sumSql.append(" where 1=1 and bodsgcode='").append(yb.getBohsgcode()).append("' group by bodsgcode,bodshmfid,bodbillno) yd on yb.bohsgcode=yd.bodsgcode and yb.bohbillno=yd.bodbillno and yb.bohmfid=yd.bodshmfid ");	
            
            /*查询条件*/   
            StringBuffer whereStr = new StringBuffer(" where 1=1 ");
            if(!StringUtil.isBlank(yb.getBohbillno())){
            	whereStr.append(" and yb.BOHBILLNO = '").append(yb.getBohbillno()).append("'");
            }   
            ...
            if(yb.getOrder() != null && yb.getSort() != null){
            	...
            	whereStr.append(" order by bohdhrq");
            }
            sql.append(whereStr);
